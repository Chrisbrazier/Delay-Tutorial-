...
// (Our component is opaque, so we must completely fill the background with a solid colour)
{
    ...
    juce::Rectangle<int> textArea = bounds
                                            .removeFromLeft((bounds.getWidth() * 2) / 3)
                                            .removeFromBottom(bounds.getHeight() / 2).reduced(10);

    juce::ColourGradient gradient(juce::Colour(0xffe62875), textArea.toFloat().getTopLeft(),
    ...
    g.drawFittedText("CHRIS", textArea, juce::Justification::centred, 1);
